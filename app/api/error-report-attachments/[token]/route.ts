...
import { hasDatabaseUrl, readErrorReportAttachmentByToken } from "@/lib/postgres";
import { ERROR_REPORT_UPLOADS_DIR, resolveLogicalDataPath } from "@/lib/runtime-paths";
...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const attachment = await readErrorReportAttachmentByToken(token);
  if (!attachment) {
    return new Response("Not found.", { status: 404, headers: NOINDEX_HEADERS });
  }
  const filePath = resolveLogicalDataPath(attachment.storedPath);
  const uploadRoot = path.resolve(ERROR_REPORT_UPLOADS_DIR);
