...
- 未纳入表桌面端列顺序固定为：站点、网址、类型、平台判断、未入榜原因、全部时段样本、最低倍率、核验档位、公告数、用户评分、操作。“全部时段样本”必须在第 6 列，“最低倍率”必须在第 7 列。
...
- Fushengyunsuan 当前分组表应保留抓到的分组证据，排名快照采用长期 Codex 口径 `vip=0.08`，采用倍率 `0.08`。`生图临时用，但是不一定安全=0.01` 只作为详情证据展示，不参与未入榜最低倍率或排名采用倍率；如果公告区展示历史公告中的 `0.0075x`，也只作为历史公告文本保留，不应出现在当前分组表、未入榜最低倍率或排名采用倍率中。
