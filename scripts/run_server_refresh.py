...
def main() -> int:
    ...
    try:
        if use_database:
            start_analysis_run(run_id, summary={"source": "server-refresh"})
            run(["python", "scripts/import_log_batches.py", "--fail-on-error"])
            steps.append("import_log_batches")
            run(["python", "audit_proxy_multipliers.py", "--log-source", "postgres"])
            steps.append("audit_proxy_multipliers postgres")
        ...
        if use_database:
            run(["python", "audit_proxy_multipliers.py", "--log-source", "postgres"])
            steps.append("audit_proxy_multipliers postgres (post-fetch)")
